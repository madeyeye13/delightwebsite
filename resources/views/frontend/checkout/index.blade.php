...
{{-- Both @push and @stack are not wired in this layout — CSS/JS must be inline --}}
<style>
    ...
    /* Dark select — override ALL background sub-properties explicitly.
       The Tailwind Forms plugin sets its own background-image on select
       elements. Without repeating every sub-property here, the browser
       may merge our dark image with the plugin's light image and tile them. */
...
{{--
    Script MUST be here — layout has no @stack('scripts') so @push is discarded.
    Placing it before the x-data element ensures alpine:init fires before Alpine
    walks the DOM. Alpine.data() + no parentheses in x-data is the correct pattern.
--}}
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('checkoutApp', () => ({
            ...
            form: {
                contact: {
                    fullName: '', email: '', phoneCode: '+234', phone: '',
            ...
            shippingMethods: [
                { id: 'free',     name: 'Free Shipping',          description: '7–15 business days', price: 0,     badge: 'FREE',    badgeCls: 'text-[10px] bg-brand-100 text-brand-700 px-1.5 py-0.5 font-semibold' },
                { id: 'standard', name: 'Standard — DHL',         description: '3–5 business days',  price: 5000,  badge: null,      badgeCls: '' },
                { id: 'express',  name: 'Express — DHL Priority', description: '1–2 business days',  price: 12000, badge: 'FASTEST', badgeCls: 'text-[10px] bg-accent-100 text-accent-700 px-1.5 py-0.5 font-semibold' },
            ],

            // ─── CART ─────────────────────────────────────────────────────────
            // In production replace with Alpine.store('cart') or $wire.cart.
            // Each item's configType mirrors the product form's selling method
            // architecture — unitLabel() renders it correctly.
            cart: {
                items: [
                    {
                        id: 1,
                        ...
                        price: 12500,
                        qty: 2,
                        configType: 'per_length',
                        unitLabel: 'yards',
                        unitsPerOrder: 5,
                        lengthUnit: 'yards',
                        minQuantity: 1,
                        quantityStep: 1,
                        variant: { name: 'Royal Blue', hex: '#1E40AF' },
                    },
                    {
                        id: 2,
                        ...
                        price: 8500,
                        qty: 1,
                        configType: 'per_piece',
                        unitLabel: 'piece',
                        unitsPerOrder: 1,
                        minQuantity: 1,
                        quantityStep: 1,
                        variant: { name: 'Gold & Cream', hex: '#D9A21B' },
                    },
                ],

                // ─── ADD-ONS ──────────────────────────────────────────────────
                // Add-ons are full product objects — same architecture as cart items.
                // configType drives the unitLabel() helper just like regular items.
                // In production these come from product.addOns relationship.
                addOns: [
                    {
                        id: 10,
                        name: 'Brocade Lining Fabric',
                        price: 4500,
                        qty: 1,
                        configType: 'per_length',
                        unitLabel: 'yards',
                        ...
                        minQuantity: 1,
                        quantityStep: 1,
                        variant: { name: 'Ivory', hex: '#F5F0E8' },
                    },
                    {
                        id: 11,
                        name: 'Matching Sash Belt',
                        price: 3200,
                        qty: 1,
                        configType: 'per_piece',
                        unitLabel: 'piece',
            ...
            // 100% free · no API key · no account · no billing
            // Usage policy: 1 req/sec max — our 800ms debounce covers this.
            ...
            searchAddress(query) {
                ...
                // 800ms debounce — respects Nominatim 1 req/sec policy
            ...
            // Fuzzy-match Nominatim state string against Nigeria states list
            ...
            placeOrder() {
                if (!this.form.paymentMethod) return;
                this.orderNumber = 'DF-' + Date.now().toString().slice(-6);
                this.goTo(3);
            },

            incQty(item) { item.qty += (item.quantityStep || 1); },
            decQty(item) {
            ...
            // ─── UNIT LABEL ───────────────────────────────────────────────────
            // Mirrors configType from the product form architecture.
            unitLabel(item) {
            ...
            applyPromo() {
                const valid = { 'SAVE10': 10, 'WELCOME15': 15, 'FABRIC20': 20 };
                const code  = (this.promoCode || '').trim().toUpperCase();
                if (!code) { this.promoError = 'Please enter a code'; return; }
                if (valid[code]) { this.promoDiscount = valid[code]; this.promoApplied = true; this.promoError = ''; }
                else             { this.promoError = 'Invalid or expired code'; this.promoApplied = false; }
            },
            clearPromo() { this.promoApplied = false; this.promoDiscount = 0; this.promoCode = ''; this.promoError = ''; },
            ...
            shippingCost() { const m = this.shippingMethods.find(m => m.id === this.form.shippingMethod); return m ? m.price : 0; },
            getTotal()     { return this.subtotal() + this.addOnsTotal() - this.promoSavings() + this.shippingCost(); },

            fmt(n) { return '₦' + Number(n).toLocaleString('en-NG', { minimumFractionDigits: 0, maximumFractionDigits: 0 }); },
            ...
            <div class="lg:col-span-7 space-y-3">
                    ...
                    <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">
                        ...
                        <div class="px-5 py-4 space-y-3">
                            ...
                            <div>
                                ...
                                <input type="text" x-model="form.contact.fullName" class="co-field" placeholder="e.g. Amara Okafor">
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="email" x-model="form.contact.email" class="co-field" placeholder="you@example.com">
                                    <p class="text-[10px] text-neutral-400 dark:text-neutral-500 mt-1">Order updates sent here</p>
                                </div>
                                <div>
                                    ...
                                    <div class="flex">
                                        ...
                                        <input type="tel" x-model="form.contact.phone" class="co-field" style="flex:1;min-width:0" placeholder="08012345678">
                                    </div>
                                </div>
                            </div>

                            {{-- Guest password — only shows when isGuest = true --}}
                    ...
                    <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">
                        ...
                        <div class="px-5 py-4 space-y-3">
                            ...
                            <div>
                                ...
                                <select x-model="form.address.country" @change="onCountryChange()" class="co-field">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <template x-if="form.address.country === 'NG'">
                                        <select x-model="form.address.state" @change="form.address.city = ''" class="co-field">
                                ...
                                <div>
                                    ...
                                    <input type="text" x-model="form.address.city" class="co-field" placeholder="e.g. Lagos">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" x-model="form.address.postal" class="co-field"
                            ...
                            <template x-for="method in shippingMethods" :key="method.id">
                                <label class="ship-card" :class="form.shippingMethod === method.id ? 'selected' : ''">
                                    ...
                                    <span class="text-sm font-semibold text-neutral-900 dark:text-neutral-50 shrink-0"
                                          x-text="method.price === 0 ? 'Free' : fmt(method.price)"></span>
                                </label>
                            </template>
                        </div>
                    </div>
                ...
                <div class="space-y-3 step-panel">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button @click="placeOrder()"
                            ...
                            :class="!form.paymentMethod ? 'opacity-40 cursor-not-allowed' : 'hover:bg-brand-600'"
                ...
                <div class="step-panel">
                    <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 px-6 py-10">
                        ...
                        <div class="border border-neutral-200 dark:border-neutral-700 mb-6">
                            <div class="px-4 py-2.5 bg-neutral-50 dark:bg-neutral-700 border-b border-neutral-200 dark:border-neutral-600">
                                <p class="text-xs font-semibold text-neutral-600 dark:text-neutral-300">Items Ordered</p>
                            </div>
                            <div class="divide-y divide-neutral-100 dark:divide-neutral-700">
                                <template x-for="item in cart.items" :key="item.id">
                                    <div class="flex items-center gap-3 px-4 py-3">
                                        <div class="w-10 h-10 bg-neutral-100 dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-600 shrink-0 flex items-center justify-center">
                                            <svg class="w-4 h-4 text-neutral-300 dark:text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-neutral-900" x-text="item.name"></p>
                                            <p class="text-[10px] text-neutral-400 dark:text-neutral-500" x-text="unitLabel(item)"></p>
                                        </div>
                                        <span class="text-sm font-semibold text-neutral-900 dark:text-neutral-50 shrink-0" x-text="fmt(item.price * item.qty)"></span>
                                    </div>
                                </template>
                            </div>
                            <div class="px-4 py-3 bg-neutral-50 dark:bg-neutral-700 border-t border-neutral-200 dark:border-neutral-600 flex justify-between">
                                <span class="text-sm font-semibold text-neutral-700 dark:text-neutral-200">Total Paid</span>
                                <span class="font-display font-bold text-neutral-900 text-sm" x-text="fmt(getTotal())"></span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-2 justify-center">
                            ...
                            <a href="#"
                                class="inline-flex items-center justify-center gap-2 px-5 py-2.5 border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300 text-sm font-medium hover:border-neutral-400 dark:hover:border-neutral-500 transition-colors">
            ...
            <div class="lg:col-span-5" :class="currentStep === 3 ? 'hidden lg:block' : ''">
                <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 sticky top-16">
                    ...
                    <div x-show="summaryOpen" class="divide-y divide-neutral-100 dark:divide-neutral-700">
                            ...
                            <template x-for="item in cart.items" :key="item.id">
                                <div class="flex gap-3">
                                    <div class="relative shrink-0">
                                        <div class="w-14 h-14 border border-neutral-200 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-700 flex items-center justify-center">
                        ...
                        {{-- ── Recommended Add-ons ─────────────────────── --}}
                        {{-- Add-ons are full product objects (same configType  --}}
                        {{-- architecture as cart items). unitLabel() renders   --}}
                        {{-- them exactly the same way.                         --}}
                        <template x-if="cart.addOns && cart.addOns.length > 0">
                            <div class="px-5 py-3 space-y-3">
                                <p class="text-[10px] font-semibold text-neutral-400 dark:text-neutral-500 tracking-widest uppercase">Recommended Add-ons</p>
                                <template x-for="addon in cart.addOns" :key="addon.id">
                                    <div class="flex gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <template x-if="addon.variant">
                                                <div class="flex items-center gap-1.5 mt-0.5">
                                                    <div class="w-2.5 h-2.5 rounded-full border border-neutral-300" :style="'background:' + addon.variant.hex"></div>
                                                    <span class="text-[10px] text-neutral-400 dark:text-neutral-500" x-text="addon.variant.name"></span>
                                                </div>
                                            </template>
                        ...
                        <div class="px-5 py-3" x-show="currentStep < 3">
                            <template x-if="!promoApplied">
                                <div>
                                    ...
                                    <p class="text-[10px] text-neutral-400 dark:text-neutral-500 mt-1">Try: SAVE10, WELCOME15, FABRIC20</p>
                                </div>
                            </template>
                            <template x-if="promoApplied">
                        ...
                        <div class="px-5 py-3 space-y-2">
                            ...
                            <div class="flex justify-between text-xs text-neutral-500 dark:text-neutral-400">
                                <span>Shipping</span>
                                <span x-text="shippingCost() === 0 ? 'Free' : fmt(shippingCost())"></span>
                            </div>
                            ...
                            <p class="text-[10px] text-neutral-400 dark:text-neutral-500">All prices in Nigerian Naira (₦)</p>
                        </div>

    ...
    >
        ...
        >
            ...
            <div class="px-5 py-4 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Parent Category <span class="text-neutral-400 font-normal">(optional)</span></label>
                    <select
                        x-model="categoryModal.parentId"
                    ...
                    >
                        ...
                        <template x-for="cat in categories.filter(c => !c.parentId)" :key="cat.id">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-neutral-50 dark:bg-[#1a2332] rounded-lg border border-neutral-200 dark:border-neutral-800">
                ...
                <div x-show="sections.basic" class="border-t border-neutral-200 dark:border-neutral-800 px-5 py-4 space-y-4">
                    ...
                    {{-- Category + Subcategory + Add Category button --}}
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            {{-- Parent category --}}
                            <select x-model="selectedParentCategoryId" @change="onParentCategoryChange()" class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-700 rounded-lg text-neutral-900 dark:text-neutral-50 dark:bg-neutral-900 focus:ring-2 focus:ring-brand focus:border-transparent text-sm">
                            ...
                            <select
                                ...
                                @change="generateSchemaMarkup()"
                                x-show="childCategoriesOfSelected.length > 0"
                                class="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-700 rounded-lg text-neutral-900 dark:text-neutral-50 dark:bg-neutral-900 focus:ring-2 focus:ring-brand focus:border-transparent text-sm"
                            >
                                <option value="">Select subcategory</option>
                                <template x-for="sub in childCategoriesOfSelected" :key="sub.id">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div x-show="form.discountType">
                            ...
                            <div class="px-3 py-2 bg-neutral-100 dark:bg-neutral-900 rounded-lg text-sm font-medium text-neutral-900 dark:text-neutral-50">
                                ₦<span x-text="(calculateFinalPrice() || 0).toLocaleString()"></span>
                            </div>
                        </div>
                    </div>
                    <div>
                        ...
                        <p class="text-xs text-neutral-600 dark:text-neutral-400 mt-1" x-show="form.cost && form.price">Gross profit: ₦<span x-text="((form.price || 0) - (form.cost || 0)).toLocaleString()"></span> (<span x-text="(((form.price || 0) - (form.cost || 0)) / (form.price || 1) * 100).toFixed(1)"></span>%)</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-neutral-50 dark:bg-[#1a2332] rounded-lg border border-neutral-200 dark:border-neutral-800 p-4 space-y-3">
                ...
                <div class="space-y-2 text-xs">
                    ...
                    <div class="flex justify-between border-t border-neutral-200 dark:border-neutral-800 pt-2">
                        ...
                        <span class="font-bold text-neutral-900 dark:text-neutral-50">₦<span x-text="(calculateFinalPrice() || 0).toLocaleString()"></span></span>
                    </div>
                    <template x-if="form.coupons.length > 0">
...
function productFormManager() {
    return {
        ...
        // ─────────────────────────────────────────────────────────
        sellingMethods: window.__productFormData?.sellingMethods || [],

        // ─────────────────────────────────────────────────────────
        // CATEGORIES — dynamic list supporting inline creation
        // ─────────────────────────────────────────────────────────
        categories: window.__productFormData?.categories || [],
    ...
    // Set parent category dropdown from existing categoryId
    this.initParentCategoryFromForm();

    // Watch sellingMethodId and react immediately when it changes
    this.$watch('form.sellingMethodId', () => {
        this.onSellingMethodChange();
    ...
    this.generateSchemaMarkup();
        ...
        get flatCategories() {
            ...
            parents.forEach(p => {
                ...
                const children = this.categories.filter(c => c.parentId == p.id);
                children.forEach(ch => {
                    result.push({ id: ch.id, label: '— ' + ch.name });
                });
            });
            return result;
        },
        ...
        // ─────────────────────────────────────────────────────────
        // LINKED CATEGORY DROPDOWNS: parent → subcategory
        // ─────────────────────────────────────────────────────────
        get parentCategories() {
            return this.categories.filter(c => !c.parentId);
        },

        get childCategoriesOfSelected() {
            if (!this.selectedParentCategoryId) return [];
            return this.categories.filter(c => c.parentId == this.selectedParentCategoryId);
        },

        onParentCategoryChange() {
            var children = this.childCategoriesOfSelected;
            if (children.length === 0) {
                // No subcategories — set categoryId to the parent itself
                this.form.categoryId = this.selectedParentCategoryId;
            } else {
                // Has subcategories — clear categoryId so user must pick one
                this.form.categoryId = '';
            }
            this.generateSchemaMarkup();
        },

        initParentCategoryFromForm() {
            // Called on init to set selectedParentCategoryId from existing form.categoryId
            if (!this.form.categoryId) return;
            var cat = this.categories.find(c => c.id == this.form.categoryId);
            if (!cat) return;
            if (cat.parentId) {
                // It's a child category — set parent
                this.selectedParentCategoryId = cat.parentId;
            } else {
                // It's a parent category — check if it has children
                var hasChildren = this.categories.some(c => c.parentId == cat.id);
                this.selectedParentCategoryId = cat.id;
                if (!hasChildren) {
                    // Parent with no children — categoryId stays as parent
                }
            }
        },
        ...
        // ─────────────────────────────────────────────────────────
        openCategoryModal() {
            this.categoryModal = { parentId: '', name: '', slug: '', slugEdited: false, description: '', isActive: true };
            this.modals.category = true;
        },
        async createCategory(createAnother) {
            const { parentId, name, slug, description, isActive } = this.categoryModal;
            ...
            try {
                ...
                // Auto-select the newly created category and update parent dropdown
                this.form.categoryId = newCat.id;
                if (newCat.parentId) {
                    this.selectedParentCategoryId = newCat.parentId;
                } else {
                    this.selectedParentCategoryId = newCat.id;
                }
                this.generateSchemaMarkup();
                if (createAnother) {
                    this.categoryModal = { parentId: '', name: '', slug: '', slugEdited: false, description: '', isActive: true };
        ...
        async createSellingMethod(createAnother) {
            ...
            try {
                const newMethod = await this.$wire.storeSellingMethod({
                    name: name.trim(),
                    slug,
                    description,
                ...
                alert('Failed to create selling method: ' + (e.message || e));
            }
        },
    ...
    return price;
        ...
        addColorVariant() {
            this.form.colorVariants.push({
                ...
                mainImagePreview: null, mainImageFile: null, mainImageMediaId: null,
        ...
        handleVariantMainImage(event, vIdx) {
            ...
            reader.onload = (e) => {
                ...
                this.form.colorVariants[vIdx].mainImageMediaId = null;
            };
            reader.readAsDataURL(file);
        },
        ...
        handleMediaSelected(detail) {
            ...
            if (target === 'mainImage') {
                ...
                this.form.mainImageFile = null;
                this.form.mainImageMediaId = mediaItem.id || null;
                this.generateSchemaMarkup();
                ...
                if (vIdx >= 0 && vIdx < this.form.colorVariants.length) {
                    ...
                    this.form.colorVariants[vIdx].mainImageFile = null;
                    this.form.colorVariants[vIdx].mainImageMediaId = mediaItem.id || null;
                }
            }
        },
       ...
       async saveDraft() {
            ...
            try {
                const r = await this.$wire.saveProduct(payload, 'draft');
        ...
        async publishProduct() {
            ...
            try {
                const r = await this.$wire.saveProduct(payload, 'active');
                if (r?.redirectUrl) { window.location.href = r.redirectUrl; }
            } catch (e) {
        ...
        buildPayload(status) {
            ...
            return {
                ...
                // Images — base64 strings (drag-drop) or media library URL picks
                main_image:          this.form.mainImagePreview || null,
                main_image_media_id: this.form.mainImageMediaId || null,
                thumbnails:          this.form.thumbnails.map(t => ({ preview: t.preview, file: null })),
                ...
                color_variants: this.form.colorVariants.map((v, i) => ({
                    ...
                    main_image:       v.mainImagePreview || null,
